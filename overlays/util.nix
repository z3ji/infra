self: super: {
  util = rec {
    ...
    mapHosts = f: builtins.map f (util.dirEntries ../hosts);

    # Function to map over hosts attributes
    mapHostAttrs = key_f: val_f:
      builtins.listToAttrs (util.mapHosts (host: {
        name = key_f host;
        value = val_f host;
      }));
    ...
    nixFilesIn = path:
      builtins.map
      (e: import (path + "/${e}"))
      (builtins.filter (x: (builtins.match ".*\\.nix$" x) != null) (util.dirEntries path));
